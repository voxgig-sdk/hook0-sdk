...
class Hook0RetryFeature extends Hook0BaseFeature
{
    ...
    public function init(Hook0Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (Hook0Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(Hook0Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(Hook0Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
