...
class LoginEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = Hook0SDK::test(null, null);
        $ent = $testsdk->Login(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = login_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "login." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HOOK__TEST_LOGIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $login_ref01_ent = $client->Login(null);
...
function login_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/login/LoginTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["login01", "login02", "login03"] as $k) {
    ...
    $entid_env_raw = getenv("HOOK__TEST_LOGIN_ENTID");
