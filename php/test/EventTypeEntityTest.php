...
class EventTypeEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = Hook0SDK::test(null, null);
        $ent = $testsdk->EventType(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->EventType(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->EventType(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = event_type_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "event_type." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HOOK__TEST_EVENT_TYPE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $event_type_ref01_ent = $client->EventType(null);
...
function event_type_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/event_type/EventTypeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["event_type01", "event_type02", "event_type03"] as $k) {
    ...
    $entid_env_raw = getenv("HOOK__TEST_EVENT_TYPE_ENTID");
