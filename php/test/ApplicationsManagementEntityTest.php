...
class ApplicationsManagementEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HOOK0_TEST_APPLICATIONS_MANAGEMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function applications_management_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("HOOK0_TEST_APPLICATIONS_MANAGEMENT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["HOOK0_TEST_APPLICATIONS_MANAGEMENT_ENTID"]);
