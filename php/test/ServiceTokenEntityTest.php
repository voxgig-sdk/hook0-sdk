...
class ServiceTokenEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = Hook0SDK::test(null, null);
        $ent = $testsdk->ServiceToken(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->ServiceToken(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->ServiceToken(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = service_token_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "service_token." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HOOK__TEST_SERVICE_TOKEN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $service_token_ref01_ent = $client->ServiceToken(null);
        ...
        $service_token_ref01_markdef_up0_value = "Mark01-service_token_ref01_" . $setup["now"];
        ...
        $service_token_ref01_resdata_up0 = Helpers::to_map($service_token_ref01_resdata_up0_result);
...
function service_token_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/service_token/ServiceTokenTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["service_token01", "service_token02", "service_token03"] as $k) {
    ...
    $entid_env_raw = getenv("HOOK__TEST_SERVICE_TOKEN_ENTID");
