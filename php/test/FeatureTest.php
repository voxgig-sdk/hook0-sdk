...
class FtCtrl extends Hook0Control
{
...
class FtClient
{
    ...
    private Hook0Utility $utility;

    public function __construct(Hook0Utility $utility, array $options)
    {
    ...
    public function get_utility(): Hook0Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (Hook0Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (Hook0Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = Hook0Config::make_feature($name);
    ...
    private static function buildUrl(Hook0Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof Hook0Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new Hook0Response($fetched);
        }
    ...
    private function fail(Hook0Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (Hook0Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
