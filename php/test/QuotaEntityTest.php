...
class QuotaEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = Hook0SDK::test(null, null);
        $ent = $testsdk->Quota(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = quota_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "quota." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HOOK0_TEST_QUOTA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $quota_ref01_data = null;
        if (count($quota_ref01_data_raw) > 0) {
...
function quota_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/quota/QuotaTestData.json';
    ...
    $entid_env_raw = getenv("HOOK0_TEST_QUOTA_ENTID");
