...
class EventsManagementEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HOOK__TEST_EVENTS_MANAGEMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $events_management_ref01_data = Helpers::to_map($events_management_ref01_data_result);
...
function events_management_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("HOOK__TEST_EVENTS_MANAGEMENT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["HOOK__TEST_EVENTS_MANAGEMENT_ENTID"]);
