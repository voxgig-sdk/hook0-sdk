...
describe('ProblemEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'problem.' + op, live)) return
    }
    ...
    let problem_ref01_data = Object.values(setup.data.existing.problem)[0] as any

    // LIST
    const problem_ref01_ent = client.Problem()
    const problem_ref01_match: any = {}
