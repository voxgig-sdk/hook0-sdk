...
describe('SubscriptionDirect', async () => {
  ...
  test('direct-load-subscription', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-subscription', setup.live)) return
  ...
  test('direct-list-subscription', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-subscription', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HOOK__TEST_SUBSCRIPTION_ENTID']
