...
describe('ServiceTokenDirect', async () => {
  ...
  test('direct-load-service_token', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-service_token', setup.live)) return
  ...
  test('direct-list-service_token', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-service_token', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HOOK__TEST_SERVICE_TOKEN_ENTID']
