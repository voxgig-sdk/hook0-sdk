...
describe('Hook0Entity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'hook0.' + op, live)) return
    }
    ...
    let hook0_ref01_data = Object.values(setup.data.existing.hook0)[0] as any

    // LIST
    const hook0_ref01_ent = client.Hook0()
    const hook0_ref01_match: any = {}

    const hook0_ref01_list = (await hook0_ref01_ent.list(hook0_ref01_match)).map((e: any) => e.data())
