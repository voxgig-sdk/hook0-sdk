...
describe('InstanceEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let instance_ref01_data = Object.values(setup.data.existing.instance)[0] as any

    // LOAD
    const instance_ref01_ent = client.Instance()
    const instance_ref01_match_dt0: any = {}
    const instance_ref01_data_dt0 = await instance_ref01_ent.load(instance_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['HOOK__TEST_INSTANCE_ENTID']
  ...
  idmap = env['HOOK__TEST_INSTANCE_ENTID']
