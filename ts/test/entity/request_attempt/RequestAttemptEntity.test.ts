...
describe('RequestAttemptEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'request_attempt.' + op, live)) return
    }
    ...
    let request_attempt_ref01_data = Object.values(setup.data.existing.request_attempt)[0] as any

    // LIST
    const request_attempt_ref01_ent = client.RequestAttempt()
    const request_attempt_ref01_match: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['HOOK__TEST_REQUEST_ATTEMPT_ENTID']
