...
describe('RequestAttemptDirect', async () => {
  ...
  test('direct-load-request_attempt', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-request_attempt', setup.live)) return
  ...
  test('direct-list-request_attempt', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-request_attempt', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HOOK__TEST_REQUEST_ATTEMPT_ENTID']
