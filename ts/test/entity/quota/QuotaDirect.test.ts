...
describe('QuotaDirect', async () => {
  ...
  test('direct-load-quota', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-quota', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HOOK__TEST_QUOTA_ENTID']
