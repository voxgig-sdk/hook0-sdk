...
class EventsManagementEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EventsManagementEntity(self._client, opts)
    ...
    def data_get(self) -> EventsManagement:
    ...
    def match_get(self) -> EventsManagement:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[EventsManagement]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.EventsManagement().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def remove(self, reqmatch=None, ctrl=None) -> EventsManagement:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.EventsManagement().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
