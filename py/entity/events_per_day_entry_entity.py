...
class EventsPerDayEntryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EventsPerDayEntryEntity(self._client, opts)
    ...
    def data_get(self) -> EventsPerDayEntry:
    ...
    def match_get(self) -> EventsPerDayEntry:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[EventsPerDayEntry]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.EventsPerDayEntry().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
