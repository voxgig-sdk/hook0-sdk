...
class ApplicationsManagementEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ApplicationsManagementEntity(self._client, opts)
    ...
    def data_get(self) -> ApplicationsManagement:
    ...
    def match_get(self) -> ApplicationsManagement:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ApplicationsManagement:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ApplicationsManagement().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
