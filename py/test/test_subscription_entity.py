...
class TestSubscriptionEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Subscription(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Subscription(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _subscription_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "subscription." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOOK__TEST_SUBSCRIPTION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        subscription_ref01_ent = client.Subscription(None)
        subscription_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.subscription"), "subscription_ref01"))

        subscription_ref01_data = helpers.to_map(subscription_ref01_ent.create(subscription_ref01_data, None))
        ...
        subscription_ref01_markdef_up0_value = "Mark01-subscription_ref01_" + str(setup["now"])
        subscription_ref01_data_up0_up[subscription_ref01_markdef_up0_name] = subscription_ref01_markdef_up0_value

        subscription_ref01_resdata_up0 = helpers.to_map(subscription_ref01_ent.update(subscription_ref01_data_up0_up, None))
...
def _subscription_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/subscription/SubscriptionTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOOK__TEST_SUBSCRIPTION_ENTID"))
