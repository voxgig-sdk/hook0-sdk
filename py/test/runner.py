...
class Hook0TestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            Hook0TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = Hook0TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = Hook0TestRunner.getenv("HOOK0_TEST_LIVE")
        override = Hook0TestRunner.getenv("HOOK0_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = Hook0TestRunner.getenv(key)
        ...
        explain = Hook0TestRunner.getenv("HOOK0_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HOOK0_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if Hook0TestRunner._test_control is not None:
            return Hook0TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                Hook0TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            Hook0TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = Hook0TestRunner.load_test_control()
...
def load_env_local():
    Hook0TestRunner.load_env_local()


def env_override(m):
    return Hook0TestRunner.env_override(m)


def entity_data(v):
    return Hook0TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return Hook0TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return Hook0TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return Hook0TestRunner.load_test_control()


def live_delay_ms():
    return Hook0TestRunner.live_delay_ms()
