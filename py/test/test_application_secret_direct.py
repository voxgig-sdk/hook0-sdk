...
class TestApplicationSecretDirect:

    def test_should_direct_list_application_secret(self):
        setup = _application_secret_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-application_secret", "live" if setup["live"] else "unit")
        if _skip:
...
def _application_secret_direct_setup(mockres):
