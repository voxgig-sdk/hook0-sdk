...
class TestHealthDirect:
    ...
    def test_should_direct_load_health(self):
        setup = _health_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-health", "live" if setup["live"] else "unit")
        if _skip:
...
def _health_direct_setup(mockres):
