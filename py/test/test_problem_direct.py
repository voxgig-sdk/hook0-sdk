...
class TestProblemDirect:

    def test_should_direct_list_problem(self):
        setup = _problem_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-problem", "live" if setup["live"] else "unit")
        if _skip:
...
def _problem_direct_setup(mockres):
