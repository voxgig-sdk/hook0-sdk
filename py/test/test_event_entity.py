...
class TestEventEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Event(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Event(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOOK__TEST_EVENT_ENTID JSON to run live")
        client = setup["client"]
...
def _event_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/event/EventTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOOK__TEST_EVENT_ENTID"))
