...
class TestApplicationsManagementEntity:

    def test_should_create_instance(self):
        testsdk = Hook0SDK.test(None, None)
        ent = testsdk.ApplicationsManagement(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _applications_management_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "applications_management." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOOK__TEST_APPLICATIONS_MANAGEMENT_ENTID JSON to run live")
        client = setup["client"]
        ...
        applications_management_ref01_data = None
        if len(applications_management_ref01_data_raw) > 0:
            applications_management_ref01_data = helpers.to_map(applications_management_ref01_data_raw[0][1])



def _applications_management_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/applications_management/ApplicationsManagementTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "HOOK__TEST_APPLICATIONS_MANAGEMENT_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOOK__TEST_APPLICATIONS_MANAGEMENT_ENTID"))
