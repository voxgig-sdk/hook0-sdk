...
class TestHook0Direct:

    def test_should_direct_list_hook0(self):
        setup = _hook0_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-hook0", "live" if setup["live"] else "unit")
        if _skip:
...
def _hook0_direct_setup(mockres):
