    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOOK0_TEST_REGISTRATION_ENTID JSON to run live")
        client = setup["client"]
        ...
        registration_ref01_data = helpers.to_map(runner.entity_data(registration_ref01_ent.create(registration_ref01_data, None)))
...
def _registration_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOOK0_TEST_REGISTRATION_ENTID"))
