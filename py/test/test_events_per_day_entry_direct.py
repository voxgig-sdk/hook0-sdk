...
class TestEventsPerDayEntryDirect:

    def test_should_direct_list_events_per_day_entry(self):
        setup = _events_per_day_entry_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-events_per_day_entry", "live" if setup["live"] else "unit")
        if _skip:
...
def _events_per_day_entry_direct_setup(mockres):
