...
class TestUserInvitationEntity:

    def test_should_create_instance(self):
        testsdk = Hook0SDK.test(None, None)
        ent = testsdk.UserInvitation(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _user_invitation_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "user_invitation." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOOK0_TEST_USER_INVITATION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        user_invitation_ref01_ent = client.UserInvitation(None)
        ...
        user_invitation_ref01_data = helpers.to_map(runner.entity_data(user_invitation_ref01_ent.create(user_invitation_ref01_data, None)))
        assert user_invitation_ref01_data is not None



def _user_invitation_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/user_invitation/UserInvitationTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "HOOK0_TEST_USER_INVITATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOOK0_TEST_USER_INVITATION_ENTID"))
