...
func TestApplicationDirect(t *testing.T) {
	t.Run("direct-list-application", func(t *testing.T) {
		setup := applicationDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-application", _mode); _shouldSkip {
			if _reason == "" {
...
func applicationDirectSetup(mockres any) *applicationDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"HOOK__TEST_APPLICATION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HOOK__TEST_APPLICATION_ENTID"]; ok {
