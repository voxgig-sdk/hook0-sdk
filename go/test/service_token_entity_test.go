...
func TestServiceTokenEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HOOK__TEST_SERVICE_TOKEN_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		serviceTokenRef01Data = core.ToMapAny(serviceTokenRef01DataResult)
		...
		serviceTokenRef01ResdataUp0 := core.ToMapAny(serviceTokenRef01ResdataUp0Result)
...
func service_tokenBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HOOK__TEST_SERVICE_TOKEN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HOOK__TEST_SERVICE_TOKEN_ENTID"])
