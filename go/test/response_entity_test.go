...
func TestResponseEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HOOK__TEST_RESPONSE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		responseRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.response", setup.data)))
...
func responseBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HOOK__TEST_RESPONSE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HOOK__TEST_RESPONSE_ENTID"])
