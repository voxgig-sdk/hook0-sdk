...
func TestResponseEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HOOK0_TEST_RESPONSE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func responseBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HOOK0_TEST_RESPONSE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HOOK0_TEST_RESPONSE_ENTID"])
