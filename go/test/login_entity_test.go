...
func TestLoginEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HOOK0_TEST_LOGIN_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		loginRef01Data = core.ToMapAny(entityData(loginRef01DataResult))
...
func loginBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HOOK0_TEST_LOGIN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HOOK0_TEST_LOGIN_ENTID"])
