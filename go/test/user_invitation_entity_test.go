...
func TestUserInvitationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HOOK0_TEST_USER_INVITATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		userInvitationRef01Data = core.ToMapAny(entityData(userInvitationRef01DataResult))
...
func user_invitationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HOOK0_TEST_USER_INVITATION_ENTID")
