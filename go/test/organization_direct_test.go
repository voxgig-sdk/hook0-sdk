...
func TestOrganizationDirect(t *testing.T) {
	t.Run("direct-list-organization", func(t *testing.T) {
		setup := organizationDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-organization", _mode); _shouldSkip {
			if _reason == "" {
...
func organizationDirectSetup(mockres any) *organizationDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"HOOK0_TEST_ORGANIZATION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HOOK0_TEST_ORGANIZATION_ENTID"]; ok {
