...
func TestApplicationSecretEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.ApplicationSecret(nil)
		if ent == nil {
			t.Fatal("expected non-nil ApplicationSecretEntity")
		}
	})
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.ApplicationSecret(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ApplicationSecret(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		setup := application_secretBasicSetup(nil)
		...
		for _, _op := range []string{"create", "list", "update"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "application_secret." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		applicationSecretRef01Data = core.ToMapAny(applicationSecretRef01DataResult)
		...
		applicationSecretRef01ListResult, err := applicationSecretRef01Ent.List(applicationSecretRef01Match, nil)
		if err != nil {
		...
		applicationSecretRef01MarkdefUp0Value := fmt.Sprintf("Mark01-application_secret_ref01_%d", setup.now)
		applicationSecretRef01DataUp0Up[applicationSecretRef01MarkdefUp0Name] = applicationSecretRef01MarkdefUp0Value

		applicationSecretRef01ResdataUp0Result, err := applicationSecretRef01Ent.Update(applicationSecretRef01DataUp0Up, nil)
		if err != nil {
			t.Fatalf("update failed: %v", err)
		}
		applicationSecretRef01ResdataUp0 := core.ToMapAny(applicationSecretRef01ResdataUp0Result)
...
func application_secretBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "application_secret", "ApplicationSecretTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read application_secret test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse application_secret test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("HOOK__TEST_APPLICATION_SECRET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HOOK__TEST_APPLICATION_SECRET_ENTID"])
