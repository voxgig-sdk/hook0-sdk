...
func TestApplicationSecretEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HOOK0_TEST_APPLICATION_SECRET_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		applicationSecretRef01Data = core.ToMapAny(entityData(applicationSecretRef01DataResult))
...
func application_secretBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HOOK0_TEST_APPLICATION_SECRET_ENTID")
