...
func NewHealthEntity(client *core.Hook0SDK, entopts map[string]any) *HealthEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *HealthEntity) GetName() string { return e.name }
...
func (e *HealthEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *HealthEntity) Deleted() bool {
	return e.deleted
}


func (e *HealthEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewHealthEntity(e.client, opts)
}

func (e *HealthEntity) Data(args ...any) any {
...
func (e *HealthEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Health; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *HealthEntity) DataTyped(data ...Health) Health {
	if len(data) > 0 {
		return typedFrom[Health](e.Data(asMap(data[0])))
	}
	return typedFrom[Health](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Health (all fields
// optional at the wire level).
func (e *HealthEntity) MatchTyped(match ...Health) Health {
	if len(match) > 0 {
		return typedFrom[Health](e.Match(asMap(match[0])))
	}
	return typedFrom[Health](e.Match())
}
...
func (e *HealthEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *HealthEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// HealthLoadMatch and returns an Health. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *HealthEntity) LoadTyped(reqmatch HealthLoadMatch, ctrl map[string]any) (Health, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return Health{}, err
	}
	return typedFrom[Health](res), nil
}



func (e *HealthEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *HealthEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *HealthEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *HealthEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *HealthEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
