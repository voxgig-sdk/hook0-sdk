...
describe("LoginEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HOOK0_TEST_LOGIN_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    login_ref01_data = helpers.to_map(type(login_ref01_data_result) == 'table' and login_ref01_data_result.data_get and login_ref01_data_result:data_get() or login_ref01_data_result)
...
function login_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HOOK0_TEST_LOGIN_ENTID")
