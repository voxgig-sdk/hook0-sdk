...
describe("EventEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Event(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Event(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HOOK__TEST_EVENT_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(event_ref01_data_dt0_loaded)
...
function event_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/event/EventTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read event test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("HOOK__TEST_EVENT_ENTID")
  local idmap_overridden = entid_env_raw ~= nil and entid_env_raw:match("^%s*{") ~= nil

  local env = runner.env_override({
    ["HOOK__TEST_EVENT_ENTID"] = idmap,
