...
describe("EventTypeDirect", function()
  it("should direct-list-event_type", function()
    local setup = event_type_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-event_type", setup.live and "live" or "unit")
  ...
  it("should direct-load-event_type", function()
    local setup = event_type_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-event_type", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function event_type_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["HOOK0_TEST_EVENT_TYPE_ENTID"] = {},
