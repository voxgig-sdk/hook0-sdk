...
describe("OrganizationDirect", function()
  it("should direct-list-organization", function()
    local setup = organization_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-organization", setup.live and "live" or "unit")
  ...
  it("should direct-load-organization", function()
    local setup = organization_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-organization", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function organization_direct_setup(mockres)
