...
describe("SubscriptionEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Subscription(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Subscription(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Subscription(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = subscription_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "subscription." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HOOK__TEST_SUBSCRIPTION_ENTID JSON to run live")
      return
    end
    ...
    local subscription_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.subscription"), "subscription_ref01"))

    local subscription_ref01_data_result, err = subscription_ref01_ent:create(subscription_ref01_data, nil)
    assert.is_nil(err)
    ...
    local subscription_ref01_list_result, err = subscription_ref01_ent:list(subscription_ref01_match, nil)
    assert.is_nil(err)
    ...
    local subscription_ref01_markdef_up0_value = "Mark01-subscription_ref01_" .. tostring(setup.now)
    subscription_ref01_data_up0_up[subscription_ref01_markdef_up0_name] = subscription_ref01_markdef_up0_value

    local subscription_ref01_resdata_up0_result, err = subscription_ref01_ent:update(subscription_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local subscription_ref01_resdata_up0 = helpers.to_map(subscription_ref01_resdata_up0_result)
...
function subscription_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/subscription/SubscriptionTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read subscription test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("HOOK__TEST_SUBSCRIPTION_ENTID")
