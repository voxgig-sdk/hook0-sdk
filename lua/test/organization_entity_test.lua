...
describe("OrganizationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HOOK__TEST_ORGANIZATION_ENTID JSON to run live")
      return
    end
    ...
    local organization_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.organization"), "organization_ref01"))

    local organization_ref01_data_result, err = organization_ref01_ent:create(organization_ref01_data, nil)
    assert.is_nil(err)
    organization_ref01_data = helpers.to_map(organization_ref01_data_result)
    ...
    local organization_ref01_resdata_up0_result, err = organization_ref01_ent:update(organization_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local organization_ref01_resdata_up0 = helpers.to_map(organization_ref01_resdata_up0_result)
...
function organization_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HOOK__TEST_ORGANIZATION_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["HOOK__TEST_ORGANIZATION_ENTID"])
