...
describe("EventDirect", function()
  it("should direct-list-event", function()
    local setup = event_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-event", setup.live and "live" or "unit")
  ...
  it("should direct-load-event", function()
    local setup = event_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-event", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function event_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["HOOK__TEST_EVENT_ENTID"] = {},
