...
describe("RequestAttemptDirect", function()
  it("should direct-list-request_attempt", function()
    local setup = request_attempt_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-request_attempt", setup.live and "live" or "unit")
  ...
  it("should direct-load-request_attempt", function()
    local setup = request_attempt_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-request_attempt", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function request_attempt_direct_setup(mockres)
