...
describe("ApplicationDirect", function()
  it("should direct-list-application", function()
    local setup = application_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-application", setup.live and "live" or "unit")
  ...
  it("should direct-load-application", function()
    local setup = application_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-application", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function application_direct_setup(mockres)
