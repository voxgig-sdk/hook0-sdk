...
describe("HealthEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HOOK__TEST_HEALTH_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local health_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.health")))
    local health_ref01_data = nil
    if #health_ref01_data_raw > 0 then
      health_ref01_data = helpers.to_map(health_ref01_data_raw[1][2])
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(health_ref01_data_dt0_loaded)
...
function health_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HOOK__TEST_HEALTH_ENTID")
