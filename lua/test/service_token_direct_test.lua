...
describe("ServiceTokenDirect", function()
  it("should direct-list-service_token", function()
    local setup = service_token_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-service_token", setup.live and "live" or "unit")
  ...
  it("should direct-load-service_token", function()
    local setup = service_token_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-service_token", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function service_token_direct_setup(mockres)
