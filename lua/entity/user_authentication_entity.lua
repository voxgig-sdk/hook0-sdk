-- Hook0 SDK UserAuthentication entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local UserAuthenticationEntity = {}
UserAuthenticationEntity.__index = UserAuthenticationEntity


function UserAuthenticationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserAuthenticationEntity)
  self._name = "user_authentication"
  self._client = client
...
function UserAuthenticationEntity:get_name()
  return self._name
end


function UserAuthenticationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserAuthenticationEntity.new(self._client, opts)
end
...
function UserAuthenticationEntity:mark_deleted()
  self._deleted = true
end


function UserAuthenticationEntity:deleted()
  return true == self._deleted
end


function UserAuthenticationEntity:data_set(args)
  if args ~= nil then
...
function UserAuthenticationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserAuthenticationEntity:match_set(args)
  if args ~= nil then
...
function UserAuthenticationEntity:match_get()
...
function UserAuthenticationEntity:stream(action, args, callopts)
...
---@param reqdata UserAuthenticationCreateData
---@param ctrl? table
---@return UserAuthentication
---@return string? err
function UserAuthenticationEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function UserAuthenticationEntity:_run_op(ctx, post_done)
  local utility = self._utility
