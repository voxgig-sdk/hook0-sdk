-- Hook0 SDK Instance entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InstanceEntity = {}
InstanceEntity.__index = InstanceEntity


function InstanceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InstanceEntity)
  self._name = "instance"
  self._client = client
...
function InstanceEntity:get_name()
  return self._name
end


function InstanceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InstanceEntity.new(self._client, opts)
end
...
function InstanceEntity:mark_deleted()
  self._deleted = true
end


function InstanceEntity:deleted()
  return true == self._deleted
end


function InstanceEntity:data_set(args)
  if args ~= nil then
...
function InstanceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InstanceEntity:match_set(args)
  if args ~= nil then
...
function InstanceEntity:match_get()
...
function InstanceEntity:stream(action, args, callopts)
...
---@param reqmatch InstanceLoadMatch
---@param ctrl? table
---@return Instance
---@return string? err
function InstanceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function InstanceEntity:_run_op(ctx, post_done)
  local utility = self._utility
