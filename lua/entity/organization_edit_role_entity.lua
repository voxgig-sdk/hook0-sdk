-- Hook0 SDK OrganizationEditRole entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local OrganizationEditRoleEntity = {}
OrganizationEditRoleEntity.__index = OrganizationEditRoleEntity


function OrganizationEditRoleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OrganizationEditRoleEntity)
  self._name = "organization_edit_role"
  self._client = client
...
function OrganizationEditRoleEntity:get_name()
  return self._name
end


function OrganizationEditRoleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OrganizationEditRoleEntity.new(self._client, opts)
end
...
function OrganizationEditRoleEntity:mark_deleted()
  self._deleted = true
end


function OrganizationEditRoleEntity:deleted()
  return true == self._deleted
end


function OrganizationEditRoleEntity:data_set(args)
  if args ~= nil then
...
function OrganizationEditRoleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OrganizationEditRoleEntity:match_set(args)
  if args ~= nil then
...
function OrganizationEditRoleEntity:match_get()
...
function OrganizationEditRoleEntity:stream(action, args, callopts)
...
---@param reqdata OrganizationEditRoleUpdateData
---@param ctrl? table
---@return OrganizationEditRole
---@return string? err
function OrganizationEditRoleEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function OrganizationEditRoleEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return OrganizationEditRoleEntity
