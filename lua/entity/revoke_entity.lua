...
local RevokeEntity = {}
RevokeEntity.__index = RevokeEntity


function RevokeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RevokeEntity)
  self._name = "revoke"
  self._client = client
...
function RevokeEntity:get_name()
  return self._name
end


function RevokeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RevokeEntity.new(self._client, opts)
end
...
function RevokeEntity:mark_deleted()
  self._deleted = true
end


function RevokeEntity:deleted()
  return true == self._deleted
end


function RevokeEntity:data_set(args)
  if args ~= nil then
...
function RevokeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RevokeEntity:match_set(args)
  if args ~= nil then
...
function RevokeEntity:match_get()
...
function RevokeEntity:stream(action, args, callopts)
...
---@param reqmatch RevokeRemoveMatch
---@param ctrl? table
---@return Revoke
---@return string? err
function RevokeEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function RevokeEntity:_run_op(ctx, post_done)
  local utility = self._utility
