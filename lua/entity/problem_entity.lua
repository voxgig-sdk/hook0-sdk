...
local ProblemEntity = {}
ProblemEntity.__index = ProblemEntity


function ProblemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProblemEntity)
  self._name = "problem"
  self._client = client
...
function ProblemEntity:get_name()
  return self._name
end


function ProblemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProblemEntity.new(self._client, opts)
end
...
function ProblemEntity:mark_deleted()
  self._deleted = true
end


function ProblemEntity:deleted()
  return true == self._deleted
end


function ProblemEntity:data_set(args)
  if args ~= nil then
...
function ProblemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProblemEntity:match_set(args)
  if args ~= nil then
...
function ProblemEntity:match_get()
...
function ProblemEntity:stream(action, args, callopts)
...
function ProblemEntity:_run_op(ctx, post_done)
  local utility = self._utility
