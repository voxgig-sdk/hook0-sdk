-- Hook0 SDK IngestedEvent entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local IngestedEventEntity = {}
IngestedEventEntity.__index = IngestedEventEntity


function IngestedEventEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IngestedEventEntity)
  self._name = "ingested_event"
  self._client = client
...
function IngestedEventEntity:get_name()
  return self._name
end


function IngestedEventEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IngestedEventEntity.new(self._client, opts)
end
...
function IngestedEventEntity:mark_deleted()
  self._deleted = true
end


function IngestedEventEntity:deleted()
  return true == self._deleted
end


function IngestedEventEntity:data_set(args)
  if args ~= nil then
...
function IngestedEventEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IngestedEventEntity:match_set(args)
  if args ~= nil then
...
function IngestedEventEntity:match_get()
...
function IngestedEventEntity:stream(action, args, callopts)
...
---@param reqdata IngestedEventCreateData
---@param ctrl? table
---@return IngestedEvent
---@return string? err
function IngestedEventEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function IngestedEventEntity:_run_op(ctx, post_done)
  local utility = self._utility
