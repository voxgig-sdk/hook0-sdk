...
local Hook0Entity = {}
Hook0Entity.__index = Hook0Entity


function Hook0Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, Hook0Entity)
  self._name = "hook0"
  self._client = client
...
function Hook0Entity:get_name()
  return self._name
end


function Hook0Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return Hook0Entity.new(self._client, opts)
end
...
function Hook0Entity:mark_deleted()
  self._deleted = true
end


function Hook0Entity:deleted()
  return true == self._deleted
end


function Hook0Entity:data_set(args)
  if args ~= nil then
...
function Hook0Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function Hook0Entity:match_set(args)
  if args ~= nil then
...
function Hook0Entity:match_get()
...
function Hook0Entity:stream(action, args, callopts)
...
---@param reqmatch Hook0ListMatch
---@param ctrl? table
---@return Hook0[]
---@return string? err
function Hook0Entity:list(reqmatch, ctrl)
  local utility = self._utility
...
function Hook0Entity:_run_op(ctx, post_done)
  local utility = self._utility
