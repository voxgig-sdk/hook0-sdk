...
function EventEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EventEntity)
  self._name = "event"
  self._client = client
...
function EventEntity:get_name()
  return self._name
end


function EventEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EventEntity.new(self._client, opts)
end
...
function EventEntity:mark_deleted()
  self._deleted = true
end


function EventEntity:deleted()
  return true == self._deleted
end


function EventEntity:data_set(args)
  if args ~= nil then
...
function EventEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EventEntity:match_set(args)
  if args ~= nil then
...
function EventEntity:match_get()
...
function EventEntity:stream(action, args, callopts)
...
---@param reqmatch EventLoadMatch
---@param ctrl? table
---@return Event
---@return string? err
function EventEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function EventEntity:_run_op(ctx, post_done)
  local utility = self._utility
