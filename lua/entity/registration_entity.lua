-- Hook0 SDK Registration entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RegistrationEntity = {}
RegistrationEntity.__index = RegistrationEntity


function RegistrationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RegistrationEntity)
...
function RegistrationEntity:get_name()
  return self._name
end


function RegistrationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RegistrationEntity.new(self._client, opts)
end
...
function RegistrationEntity:mark_deleted()
  self._deleted = true
end


function RegistrationEntity:deleted()
  return true == self._deleted
end


function RegistrationEntity:data_set(args)
  if args ~= nil then
...
function RegistrationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RegistrationEntity:match_set(args)
  if args ~= nil then
...
function RegistrationEntity:match_get()
...
function RegistrationEntity:stream(action, args, callopts)
...
---@param reqdata RegistrationCreateData
---@param ctrl? table
---@return Registration
---@return string? err
function RegistrationEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function RegistrationEntity:_run_op(ctx, post_done)
  local utility = self._utility
