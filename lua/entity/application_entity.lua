...
local ApplicationEntity = {}
ApplicationEntity.__index = ApplicationEntity


function ApplicationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApplicationEntity)
...
function ApplicationEntity:get_name()
  return self._name
end


function ApplicationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApplicationEntity.new(self._client, opts)
end
...
function ApplicationEntity:mark_deleted()
  self._deleted = true
end


function ApplicationEntity:deleted()
  return true == self._deleted
end


function ApplicationEntity:data_set(args)
  if args ~= nil then
...
function ApplicationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApplicationEntity:match_set(args)
  if args ~= nil then
...
function ApplicationEntity:match_get()
...
function ApplicationEntity:stream(action, args, callopts)
...
---@param reqmatch ApplicationListMatch
---@param ctrl? table
---@return Application[]
---@return string? err
function ApplicationEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata ApplicationUpdateData
---@param ctrl? table
---@return Application
---@return string? err
function ApplicationEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch ApplicationRemoveMatch
---@param ctrl? table
---@return Application
---@return string? err
function ApplicationEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function ApplicationEntity:_run_op(ctx, post_done)
  local utility = self._utility
