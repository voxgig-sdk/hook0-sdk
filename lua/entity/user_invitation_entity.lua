-- Hook0 SDK UserInvitation entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local UserInvitationEntity = {}
UserInvitationEntity.__index = UserInvitationEntity


function UserInvitationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UserInvitationEntity)
  self._name = "user_invitation"
  self._client = client
...
function UserInvitationEntity:get_name()
  return self._name
end


function UserInvitationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UserInvitationEntity.new(self._client, opts)
end
...
function UserInvitationEntity:mark_deleted()
  self._deleted = true
end


function UserInvitationEntity:deleted()
  return true == self._deleted
end


function UserInvitationEntity:data_set(args)
  if args ~= nil then
...
function UserInvitationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UserInvitationEntity:match_set(args)
  if args ~= nil then
...
function UserInvitationEntity:match_get()
...
function UserInvitationEntity:stream(action, args, callopts)
...
---@param reqdata UserInvitationCreateData
---@param ctrl? table
---@return UserInvitation
---@return string? err
function UserInvitationEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function UserInvitationEntity:_run_op(ctx, post_done)
  local utility = self._utility
