-- Hook0 SDK Organization entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local OrganizationEntity = {}
OrganizationEntity.__index = OrganizationEntity


function OrganizationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OrganizationEntity)
  self._name = "organization"
  self._client = client
...
function OrganizationEntity:get_name()
  return self._name
end


function OrganizationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OrganizationEntity.new(self._client, opts)
end
...
function OrganizationEntity:mark_deleted()
  self._deleted = true
end


function OrganizationEntity:deleted()
  return true == self._deleted
end


function OrganizationEntity:data_set(args)
  if args ~= nil then
...
function OrganizationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OrganizationEntity:match_set(args)
  if args ~= nil then
...
function OrganizationEntity:match_get()
...
function OrganizationEntity:stream(action, args, callopts)
...
---@param reqdata OrganizationCreateData
---@param ctrl? table
---@return Organization
---@return string? err
function OrganizationEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata OrganizationUpdateData
---@param ctrl? table
---@return Organization
---@return string? err
function OrganizationEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch OrganizationRemoveMatch
---@param ctrl? table
---@return Organization
---@return string? err
function OrganizationEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function OrganizationEntity:_run_op(ctx, post_done)
  local utility = self._utility
