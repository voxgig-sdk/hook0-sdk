-- Hook0 SDK ApplicationsManagement entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ApplicationsManagementEntity = {}
ApplicationsManagementEntity.__index = ApplicationsManagementEntity


function ApplicationsManagementEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApplicationsManagementEntity)
  self._name = "applications_management"
  self._client = client
...
function ApplicationsManagementEntity:get_name()
  return self._name
end


function ApplicationsManagementEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApplicationsManagementEntity.new(self._client, opts)
end
...
function ApplicationsManagementEntity:mark_deleted()
  self._deleted = true
end


function ApplicationsManagementEntity:deleted()
  return true == self._deleted
end


function ApplicationsManagementEntity:data_set(args)
  if args ~= nil then
...
function ApplicationsManagementEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApplicationsManagementEntity:match_set(args)
  if args ~= nil then
...
function ApplicationsManagementEntity:match_get()
...
function ApplicationsManagementEntity:stream(action, args, callopts)
...
---@param reqmatch ApplicationsManagementRemoveMatch
---@param ctrl? table
---@return ApplicationsManagement
---@return string? err
function ApplicationsManagementEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ApplicationsManagementEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ApplicationsManagementEntity
