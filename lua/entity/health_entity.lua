...
local HealthEntity = {}
HealthEntity.__index = HealthEntity


function HealthEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HealthEntity)
  self._name = "health"
  self._client = client
...
function HealthEntity:get_name()
  return self._name
end


function HealthEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HealthEntity.new(self._client, opts)
end
...
function HealthEntity:mark_deleted()
  self._deleted = true
end


function HealthEntity:deleted()
  return true == self._deleted
end


function HealthEntity:data_set(args)
  if args ~= nil then
...
function HealthEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HealthEntity:match_set(args)
  if args ~= nil then
...
function HealthEntity:match_get()
...
function HealthEntity:stream(action, args, callopts)
...
---@param reqmatch HealthLoadMatch
---@param ctrl? table
---@return Health
---@return string? err
function HealthEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function HealthEntity:_run_op(ctx, post_done)
  local utility = self._utility
