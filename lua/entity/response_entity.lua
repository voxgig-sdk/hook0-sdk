-- Hook0 SDK Response entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ResponseEntity = {}
ResponseEntity.__index = ResponseEntity


function ResponseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResponseEntity)
  self._name = "response"
  self._client = client
...
function ResponseEntity:get_name()
  return self._name
end


function ResponseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResponseEntity.new(self._client, opts)
end
...
function ResponseEntity:mark_deleted()
  self._deleted = true
end


function ResponseEntity:deleted()
  return true == self._deleted
end


function ResponseEntity:data_set(args)
  if args ~= nil then
...
function ResponseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResponseEntity:match_set(args)
  if args ~= nil then
...
function ResponseEntity:match_get()
...
function ResponseEntity:stream(action, args, callopts)
...
---@param reqmatch ResponseLoadMatch
---@param ctrl? table
---@return Response
---@return string? err
function ResponseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ResponseEntity:_run_op(ctx, post_done)
  local utility = self._utility
