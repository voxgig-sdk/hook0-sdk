...
local EventTypeEntity = {}
EventTypeEntity.__index = EventTypeEntity


function EventTypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EventTypeEntity)
...
function EventTypeEntity:get_name()
  return self._name
end


function EventTypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EventTypeEntity.new(self._client, opts)
end
...
function EventTypeEntity:mark_deleted()
  self._deleted = true
end


function EventTypeEntity:deleted()
  return true == self._deleted
end


function EventTypeEntity:data_set(args)
  if args ~= nil then
...
function EventTypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EventTypeEntity:match_set(args)
  if args ~= nil then
...
function EventTypeEntity:match_get()
...
function EventTypeEntity:stream(action, args, callopts)
...
---@param reqmatch EventTypeLoadMatch
---@param ctrl? table
---@return EventType
---@return string? err
function EventTypeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch EventTypeListMatch
---@param ctrl? table
---@return EventType[]
---@return string? err
function EventTypeEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata EventTypeCreateData
---@param ctrl? table
---@return EventType
---@return string? err
function EventTypeEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function EventTypeEntity:_run_op(ctx, post_done)
  local utility = self._utility
