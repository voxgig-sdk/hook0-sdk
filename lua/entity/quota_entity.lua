...
local QuotaEntity = {}
QuotaEntity.__index = QuotaEntity


function QuotaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QuotaEntity)
  self._name = "quota"
  self._client = client
...
function QuotaEntity:get_name()
  return self._name
end


function QuotaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QuotaEntity.new(self._client, opts)
end
...
function QuotaEntity:mark_deleted()
  self._deleted = true
end


function QuotaEntity:deleted()
  return true == self._deleted
end


function QuotaEntity:data_set(args)
  if args ~= nil then
...
function QuotaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QuotaEntity:match_set(args)
  if args ~= nil then
...
function QuotaEntity:match_get()
...
function QuotaEntity:stream(action, args, callopts)
...
---@param reqmatch QuotaLoadMatch
---@param ctrl? table
---@return Quota
---@return string? err
function QuotaEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function QuotaEntity:_run_op(ctx, post_done)
  local utility = self._utility
