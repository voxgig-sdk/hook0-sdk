-- Hook0 SDK Login entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LoginEntity = {}
LoginEntity.__index = LoginEntity


function LoginEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LoginEntity)
  self._name = "login"
  self._client = client
...
function LoginEntity:get_name()
  return self._name
end


function LoginEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LoginEntity.new(self._client, opts)
end
...
function LoginEntity:mark_deleted()
  self._deleted = true
end


function LoginEntity:deleted()
  return true == self._deleted
end


function LoginEntity:data_set(args)
  if args ~= nil then
...
function LoginEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LoginEntity:match_set(args)
  if args ~= nil then
...
function LoginEntity:match_get()
...
function LoginEntity:stream(action, args, callopts)
...
---@param reqdata LoginCreateData
---@param ctrl? table
---@return Login
---@return string? err
function LoginEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function LoginEntity:_run_op(ctx, post_done)
  local utility = self._utility
