// Hook0 SDK utility: makeError - the single error surface of the
// pipeline. Throws the wrapped Hook0Error unless the per-call ctrl
...
func makeErrorUtil(_ ctx: Context, _ err: Error?) throws -> Value {
  ...
  var msg = "Hook0SDK: " + opname + ": " + errmsg
  ...
  let code = (e as? Hook0Error)?.code ?? ""
  let sdkErr = Hook0Error(code, msg, ctx)
