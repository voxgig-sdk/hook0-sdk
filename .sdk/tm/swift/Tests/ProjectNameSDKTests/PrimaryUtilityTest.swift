...
final class PrimaryUtilityTest: XCTestCase {
  ...
  private var client: Hook0SDK!
  private var utility: Utility!

  override func setUp() {
    super.setUp()
    primary = SdkRunner.loadPrimary()
    client = Hook0SDK.testSDK(nil, nil)
  ...
  private func makeTestCtx(_ client: Hook0SDK, _ utility: Utility,
  ...
  private func makeTestFullCtx(_ client: Hook0SDK, _ utility: Utility) -> Context {
  ...
  func testFeatureHookBasic() {
    let hookClient = Hook0SDK.testSDK(nil, nil)
  ...
  func testFeatureInitBasic() {
    let initClient = Hook0SDK.testSDK(nil, nil)
  ...
  func testFetcherBlockedTestMode() {
    ...
    let opts = VMap()
    opts.entries["system"] = .map(vm(("fetch", .nat(fetch))))
    let blockedClient = Hook0SDK(opts)
  ...
  func testMakeSpecBasic() {
    ...
    let specClient = Hook0SDK.testSDK(nil, setupOpts)
  ...
  func testPrepareAuthBasic() {
    ...
    let authClient = Hook0SDK.testSDK(nil, setupOpts)
