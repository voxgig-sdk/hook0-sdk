// Hook0 SDK test runner - shared infrastructure for the generated test
...
enum SdkRunner {
  ...
  // three dirs from this file (swift/Tests/Hook0SDKTests -> project).
  static func testJsonPath(_ file: String = #filePath) -> String {
    let dir = URL(fileURLWithPath: file)
      .deletingLastPathComponent()  // Hook0SDKTests
  ...
  static func makeCtxFromMap(_ ctxmapIn: VMap?, _ client: Hook0SDK?, _ utility: Utility?)
    -> Context
  {
    ...
    if let resMap = ctxmap.entries["result"]?.asMap {
      ...
      if let errMap = resMap.entries["err"]?.asMap, let msg = errMap.entries["message"]?.asString {
        ctx.result!.err = Hook0Error("", msg, nil)
      }
    }
  ...
  static func fixCtx(_ ctx: Context, _ client: Hook0SDK) {
  ...
  static func errFromMap(_ m: VMap?) -> Error? {
    ...
    return Hook0Error(code, msg, nil)
  }
}
