...
// Hook0 SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class Hook0MakePoint
{
    public static function call(Hook0Context $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof Hook0Error) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = Hook0Helpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = Hook0Helpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
