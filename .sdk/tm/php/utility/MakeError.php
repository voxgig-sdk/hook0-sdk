...
// Hook0 SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class Hook0MakeError
{
    public static function call(?Hook0Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new Hook0Context([], null);
        }
        $op = $ctx->op ?? new Hook0Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new Hook0Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof Hook0Error) ? $err->msg : (string)$err;
        $msg = "Hook0SDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new Hook0Error('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof Hook0Error) {
