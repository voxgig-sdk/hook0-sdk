...
// Hook0 SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        Hook0SDK $client,
        Hook0Utility $utility
    ): Hook0Context {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new Hook0Context($ctxmap, null);
    ...
    private static function fixctx(Hook0Context $ctx, Hook0SDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?Hook0Error
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new Hook0Error($code, $msg);
    }
    ...
    {
        $client = Hook0SDK::test(null, null);
    ...
    {
        $hook_client = Hook0SDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends Hook0BaseFeature {
            public $hook_fn = null;
            public function TestHook(Hook0Context $ctx): void
            {
    ...
    {
        $init_client = Hook0SDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends Hook0BaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new Hook0Spec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = Hook0SDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = Hook0SDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof Hook0Error) ? $result->err->msg : (string)$result->err;
