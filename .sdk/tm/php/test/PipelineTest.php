...
{
    private static function utility(): Hook0Utility
    {
        return new Hook0Utility();
    }
    ...
    private static function ctx(array $over = []): Hook0Context
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new Hook0Context([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new Hook0Operation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof Hook0Error) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new Hook0Operation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = Hook0MakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new Hook0Operation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = Hook0MakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new Hook0Operation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = Hook0MakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new Hook0Spec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = Hook0MakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new Hook0Response([]),
            'result' => new Hook0Result([]),
        ]);
        [, $err] = Hook0MakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new Hook0Spec(['step' => 's']),
            'response' => new Hook0Response(self::resp(404, null, ['x-a' => '1'])),
            'result' => new Hook0Result([]),
        ]);
        [, $err] = Hook0MakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new Hook0Spec(['step' => 's']),
            'response' => new Hook0Response(self::resp(200, ['v' => 1])),
            'result' => new Hook0Result([]),
        ]);
        [, $err] = Hook0MakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new Hook0Spec(['step' => 's']),
            'response' => new Hook0Response(self::resp(200, ['v' => 2])),
            'result' => new Hook0Result([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        Hook0MakeResponse::call($ctx);
    ...
    {
        $preset = new Hook0Response(self::resp(299));
        $ctx = self::ctx([
            'spec' => new Hook0Spec([]),
            'response' => new Hook0Response([]),
            'result' => new Hook0Result([]),
        ...
        [$got, $err] = Hook0MakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new Hook0Result([])]);
        [, $err] = Hook0MakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new Hook0Spec([]), 'result' => null]);
        [, $err] = Hook0MakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new Hook0Spec(['step' => 's']),
            'result' => new Hook0Result(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new Hook0Operation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = Hook0MakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new Hook0Spec(['step' => 's']),
            'result' => new Hook0Result(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new Hook0Operation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = Hook0MakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new Hook0Result(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new Hook0Spec([]),
            'result' => new Hook0Result([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = Hook0MakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new Hook0Error('boom', 'boom');
        $utility->fetcher = function (Hook0Context $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new Hook0Spec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = Hook0MakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (Hook0Context $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = Hook0MakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(Hook0Response::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (Hook0Context $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new Hook0Spec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = Hook0MakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new Hook0Response(self::resp(201));
        $ctx = self::ctx(['spec' => new Hook0Spec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = Hook0MakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = Hook0MakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = Hook0MakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new Hook0Result(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, Hook0Done::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new Hook0Result(['ok' => false])]);
        $this->expectException(Hook0Error::class);
        Hook0Done::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new Hook0Result(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', Hook0MakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new Hook0BaseFeature();
        $b = new Hook0BaseFeature();
        Hook0FeatureAdd::call($ctx, $a);
        Hook0FeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): Hook0BaseFeature
    {
        $f = new Hook0BaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): Hook0Context
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new Hook0Spec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new Hook0Response(['headers' => null]),
            'result' => new Hook0Result([]),
        ]);
        Hook0ResultHeaders::call($ctx);
