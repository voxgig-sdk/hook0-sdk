// Hook0Error: the SDK error type (mirrors go core/error.go / the rust
...
pub const Hook0Error = struct {
    ...
    pub fn make(code: []const u8, msg: []const u8) *Hook0Error {
        const e = mem.a().create(Hook0Error) catch unreachable;
        e.* = .{ .sdk = "Hook0", .code = code, .msg = msg };
        return e;
    }
};
