// Command Hook0-mcp is an MCP server exposing the Hook0
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Hook0_list  — list records of an entity
//	Hook0_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: HOOK0_APIKEY carries the API key and
	// HOOK0_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HOOK0_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HOOK0_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHook0SDK(opts)
