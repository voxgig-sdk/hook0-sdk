...
[MCP](https://modelcontextprotocol.io) server exposing the Hook0 SDK as
two agent tools — `hook0_list` and `hook0_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hook0-mcp)
make build

# 2. Provide credentials via the environment
export HOOK0_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user hook0 \
  -- /absolute/path/to/hook0-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hook0-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hook0_list: first page of records
{ "entity": "application" }
{ "entity": "application", "query": { } }

// hook0_load: one record by id
{ "entity": "application", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hook0-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hook0 \
     -- "$PWD"/dist/*/hook0-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hook0_list` and `hook0_load` tools now appear
   in new sessions. Ask the agent to *"list application using hook0"*
   and it calls `hook0_list` with `{"entity":"application"}`.
...
```sh
export HOOK0_APIKEY=sk_live_xxx            # API key
export HOOK0_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `hook0_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hook0_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HOOK0_APIKEY` | API key sent with every request. |
| `HOOK0_BASE` | Optional override of the API base URL. |
...
```sh
./hook0-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hook0_load","arguments":{"entity":"application","query":{"id":1}}}}'
```
