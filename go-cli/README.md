...
boru-driven command-line client **and** interactive REPL for the Hook0
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hook0-cli)
make build
...
./hook0-cli list application
./hook0-cli load 1 application            # {id:1} shorthand
./hook0-cli load '{id:1}' application       # explicit match map
./hook0-cli update '{name:"x"}' application
./hook0-cli list application_secret

# 5. Override the API base URL for a single call
HOOK0_BASE=https://api.example.com ./hook0-cli list application

# 6. No arguments -> interactive REPL
./hook0-cli
hook0> list application
hook0> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hook0-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hook0-cli list application
   ```
...
```sh
./hook0-cli load 1 application          # scalar shorthand for {id:1}
./hook0-cli load '{id:1}' application     # explicit match map
```
...
```sh
./hook0-cli update '{id:1,name:"new"}' application
```
...
Run with no arguments to open a REPL (prompt `hook0>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `application`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hook0-cli`. |
...
(`./hook0-cli <expr>`) and interactively (the REPL), and expressions compose the
