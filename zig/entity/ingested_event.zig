// IngestedEvent entity client (generated — mirrors the go/rust Entity fragment).
...
pub const IngestedEventEntity = struct {
    name: []const u8 = "ingested_event",
    client: *sdk.Hook0SDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *IngestedEventEntity {
        ...
        const e = h.A().create(IngestedEventEntity) catch unreachable;
        e.* = .{
            .name = "ingested_event",
            .client = client,
    ...
    fn ent_ctx(self: *IngestedEventEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *IngestedEventEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *IngestedEventEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *IngestedEventEntity, ctx: *Context, post_done: *const fn (*IngestedEventEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *IngestedEventEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *IngestedEventEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *IngestedEventEntity, ctx: *Context, post_done: *const fn (*IngestedEventEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *IngestedEventEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *IngestedEventEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return IngestedEventEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *IngestedEventEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *IngestedEventEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *IngestedEventEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *IngestedEventEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *IngestedEventEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *IngestedEventEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *IngestedEventEntity, ctx: *Context) void {
    ...
    pub fn update(self: *IngestedEventEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *IngestedEventEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
