// Revoke entity client (generated — mirrors the go/rust Entity fragment).
...
pub const RevokeEntity = struct {
    name: []const u8 = "revoke",
    client: *sdk.Hook0SDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *RevokeEntity {
        ...
        const e = h.A().create(RevokeEntity) catch unreachable;
        e.* = .{
            .name = "revoke",
            .client = client,
    ...
    fn ent_ctx(self: *RevokeEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *RevokeEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *RevokeEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *RevokeEntity, ctx: *Context, post_done: *const fn (*RevokeEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *RevokeEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *RevokeEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *RevokeEntity, ctx: *Context, post_done: *const fn (*RevokeEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *RevokeEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *RevokeEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return RevokeEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *RevokeEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *RevokeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *RevokeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *RevokeEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *RevokeEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *RevokeEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *RevokeEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
    ...
    pub fn remove(self: *RevokeEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn remove_post_done(self: *RevokeEntity, ctx: *Context) void {
