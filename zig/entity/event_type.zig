// EventType entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EventTypeEntity = struct {
    name: []const u8 = "event_type",
    client: *sdk.Hook0SDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *EventTypeEntity {
        ...
        const e = h.A().create(EventTypeEntity) catch unreachable;
        e.* = .{
            .name = "event_type",
            .client = client,
    ...
    fn ent_ctx(self: *EventTypeEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EventTypeEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *EventTypeEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *EventTypeEntity, ctx: *Context, post_done: *const fn (*EventTypeEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *EventTypeEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *EventTypeEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *EventTypeEntity, ctx: *Context, post_done: *const fn (*EventTypeEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *EventTypeEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EventTypeEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EventTypeEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EventTypeEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EventTypeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EventTypeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *EventTypeEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *EventTypeEntity, ctx: *Context) void {
    ...
    pub fn list(self: *EventTypeEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items = std.ArrayList(*EventTypeEntity).init(h.A());
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = EventTypeEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice() catch &[_]*EventTypeEntity{} };
    }
    
    fn list_post_done(self: *EventTypeEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *EventTypeEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *EventTypeEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *EventTypeEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *EventTypeEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
