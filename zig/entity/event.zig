// Event entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EventEntity = struct {
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *EventEntity {
        ...
        const e = h.A().create(EventEntity) catch unreachable;
        e.* = .{
            .name = "event",
            .client = client,
    ...
    fn ent_ctx(self: *EventEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EventEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *EventEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *EventEntity, ctx: *Context, post_done: *const fn (*EventEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *EventEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *EventEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *EventEntity, ctx: *Context, post_done: *const fn (*EventEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *EventEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EventEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EventEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EventEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EventEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EventEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *EventEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *EventEntity, ctx: *Context) void {
    ...
    pub fn list(self: *EventEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items = std.ArrayList(*EventEntity).init(h.A());
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = EventEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice() catch &[_]*EventEntity{} };
    }
    
    fn list_post_done(self: *EventEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *EventEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *EventEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *EventEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
