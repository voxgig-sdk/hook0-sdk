// Registration entity client (generated — mirrors the go/rust Entity fragment).
...
pub const RegistrationEntity = struct {
    name: []const u8 = "registration",
    client: *sdk.Hook0SDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *RegistrationEntity {
        ...
        const e = h.A().create(RegistrationEntity) catch unreachable;
        e.* = .{
            .name = "registration",
            .client = client,
    ...
    fn ent_ctx(self: *RegistrationEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *RegistrationEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *RegistrationEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *RegistrationEntity, ctx: *Context, post_done: *const fn (*RegistrationEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *RegistrationEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *RegistrationEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *RegistrationEntity, ctx: *Context, post_done: *const fn (*RegistrationEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *RegistrationEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *RegistrationEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return RegistrationEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *RegistrationEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *RegistrationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *RegistrationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *RegistrationEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *RegistrationEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *RegistrationEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *RegistrationEntity, ctx: *Context) void {
    ...
    pub fn update(self: *RegistrationEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *RegistrationEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
