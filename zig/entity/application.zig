// Application entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ApplicationEntity = struct {
    name: []const u8 = "application",
    client: *sdk.Hook0SDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *ApplicationEntity {
        ...
        const e = h.A().create(ApplicationEntity) catch unreachable;
        e.* = .{
            .name = "application",
            .client = client,
    ...
    fn ent_ctx(self: *ApplicationEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ApplicationEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ApplicationEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *ApplicationEntity, ctx: *Context, post_done: *const fn (*ApplicationEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *ApplicationEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *ApplicationEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *ApplicationEntity, ctx: *Context, post_done: *const fn (*ApplicationEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ApplicationEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ApplicationEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ApplicationEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ApplicationEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ApplicationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ApplicationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ApplicationEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *ApplicationEntity, ctx: *Context) void {
    ...
    pub fn list(self: *ApplicationEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items = std.ArrayList(*ApplicationEntity).init(h.A());
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = ApplicationEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice() catch &[_]*ApplicationEntity{} };
    }
    
    fn list_post_done(self: *ApplicationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *ApplicationEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *ApplicationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *ApplicationEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn update_post_done(self: *ApplicationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
    ...
    pub fn remove(self: *ApplicationEntity, reqmatch: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *ApplicationEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
