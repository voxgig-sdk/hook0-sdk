// Health entity client (generated — mirrors the go/rust Entity fragment).
...
pub const HealthEntity = struct {
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *HealthEntity {
        ...
        const e = h.A().create(HealthEntity) catch unreachable;
        e.* = .{
            .name = "health",
            .client = client,
    ...
    fn ent_ctx(self: *HealthEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *HealthEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *HealthEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *HealthEntity, ctx: *Context, post_done: *const fn (*HealthEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *HealthEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *HealthEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *HealthEntity, ctx: *Context, post_done: *const fn (*HealthEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *HealthEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *HealthEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return HealthEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *HealthEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *HealthEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *HealthEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *HealthEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *HealthEntity, ctx: *Context) void {
    ...
    pub fn list(self: *HealthEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *HealthEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *HealthEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *HealthEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
