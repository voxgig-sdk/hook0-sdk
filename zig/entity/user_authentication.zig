// UserAuthentication entity client (generated — mirrors the go/rust Entity fragment).
...
pub const UserAuthenticationEntity = struct {
    name: []const u8 = "user_authentication",
    client: *sdk.Hook0SDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *UserAuthenticationEntity {
        ...
        const e = h.A().create(UserAuthenticationEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *UserAuthenticationEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *UserAuthenticationEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *UserAuthenticationEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *UserAuthenticationEntity, ctx: *Context, post_done: *const fn (*UserAuthenticationEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *UserAuthenticationEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *UserAuthenticationEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *UserAuthenticationEntity, ctx: *Context, post_done: *const fn (*UserAuthenticationEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *UserAuthenticationEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *UserAuthenticationEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return UserAuthenticationEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *UserAuthenticationEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *UserAuthenticationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *UserAuthenticationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *UserAuthenticationEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *UserAuthenticationEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *UserAuthenticationEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *UserAuthenticationEntity, ctx: *Context) void {
    ...
    pub fn update(self: *UserAuthenticationEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *UserAuthenticationEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
