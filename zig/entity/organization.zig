// Organization entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OrganizationEntity = struct {
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *OrganizationEntity {
        ...
        const e = h.A().create(OrganizationEntity) catch unreachable;
        e.* = .{
            .name = "organization",
            .client = client,
    ...
    fn ent_ctx(self: *OrganizationEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OrganizationEntity, ctx: *Context) OpResult {
    ...
    pub fn stream(self: *OrganizationEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OrganizationEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OrganizationEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OrganizationEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OrganizationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OrganizationEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
