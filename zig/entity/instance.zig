// Instance entity client (generated — mirrors the go/rust Entity fragment).
...
pub const InstanceEntity = struct {
    name: []const u8 = "instance",
    client: *sdk.Hook0SDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *InstanceEntity {
        ...
        const e = h.A().create(InstanceEntity) catch unreachable;
        e.* = .{
            .name = "instance",
            .client = client,
    ...
    fn ent_ctx(self: *InstanceEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *InstanceEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *InstanceEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *InstanceEntity, ctx: *Context, post_done: *const fn (*InstanceEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *InstanceEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *InstanceEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *InstanceEntity, ctx: *Context, post_done: *const fn (*InstanceEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *InstanceEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *InstanceEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return InstanceEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *InstanceEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *InstanceEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *InstanceEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *InstanceEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *InstanceEntity, ctx: *Context) void {
    ...
    pub fn list(self: *InstanceEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *InstanceEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *InstanceEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *InstanceEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
