// EventsManagement entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EventsManagementEntity = struct {
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *EventsManagementEntity {
        ...
        const e = h.A().create(EventsManagementEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *EventsManagementEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *EventsManagementEntity, ctx: *Context) OpResult {
    ...
    pub fn stream(self: *EventsManagementEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *EventsManagementEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return EventsManagementEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *EventsManagementEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *EventsManagementEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *EventsManagementEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
