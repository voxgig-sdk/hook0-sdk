// OrganizationEditRole entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OrganizationEditRoleEntity = struct {
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *OrganizationEditRoleEntity {
        ...
        const e = h.A().create(OrganizationEditRoleEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OrganizationEditRoleEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OrganizationEditRoleEntity, ctx: *Context) OpResult {
    ...
    pub fn stream(self: *OrganizationEditRoleEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OrganizationEditRoleEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OrganizationEditRoleEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OrganizationEditRoleEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OrganizationEditRoleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OrganizationEditRoleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
