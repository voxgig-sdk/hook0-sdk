// Login entity client (generated — mirrors the go/rust Entity fragment).
...
pub const LoginEntity = struct {
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *LoginEntity {
        ...
        const e = h.A().create(LoginEntity) catch unreachable;
        e.* = .{
            .name = "login",
            .client = client,
    ...
    fn ent_ctx(self: *LoginEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *LoginEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *LoginEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *LoginEntity, ctx: *Context, post_done: *const fn (*LoginEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *LoginEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *LoginEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *LoginEntity, ctx: *Context, post_done: *const fn (*LoginEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *LoginEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *LoginEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return LoginEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *LoginEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *LoginEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *LoginEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *LoginEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *LoginEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *LoginEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *LoginEntity, ctx: *Context) void {
    ...
    pub fn update(self: *LoginEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *LoginEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
