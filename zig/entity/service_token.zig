// ServiceToken entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ServiceTokenEntity = struct {
    name: []const u8 = "service_token",
    client: *sdk.Hook0SDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *ServiceTokenEntity {
        ...
        const e = h.A().create(ServiceTokenEntity) catch unreachable;
        e.* = .{
            .name = "service_token",
            .client = client,
    ...
    fn ent_ctx(self: *ServiceTokenEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ServiceTokenEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ServiceTokenEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *ServiceTokenEntity, ctx: *Context, post_done: *const fn (*ServiceTokenEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *ServiceTokenEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *ServiceTokenEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *ServiceTokenEntity, ctx: *Context, post_done: *const fn (*ServiceTokenEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ServiceTokenEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ServiceTokenEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ServiceTokenEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ServiceTokenEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ServiceTokenEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ServiceTokenEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ServiceTokenEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *ServiceTokenEntity, ctx: *Context) void {
    ...
    pub fn list(self: *ServiceTokenEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
        ...
        var items = std.ArrayList(*ServiceTokenEntity).init(h.A());
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = ServiceTokenEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice() catch &[_]*ServiceTokenEntity{} };
    }
    
    fn list_post_done(self: *ServiceTokenEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *ServiceTokenEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *ServiceTokenEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *ServiceTokenEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn update_post_done(self: *ServiceTokenEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
    ...
    pub fn remove(self: *ServiceTokenEntity, reqmatch: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *ServiceTokenEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
