// Response entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ResponseEntity = struct {
    ...
    pub fn new(client: *sdk.Hook0SDK, entopts_in: Value) *ResponseEntity {
        ...
        const e = h.A().create(ResponseEntity) catch unreachable;
        e.* = .{
            .name = "response",
            .client = client,
    ...
    fn ent_ctx(self: *ResponseEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ResponseEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ResponseEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *ResponseEntity, ctx: *Context, post_done: *const fn (*ResponseEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *ResponseEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *ResponseEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *ResponseEntity, ctx: *Context, post_done: *const fn (*ResponseEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ResponseEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ResponseEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ResponseEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ResponseEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ResponseEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ResponseEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ResponseEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *ResponseEntity, ctx: *Context) void {
    ...
    pub fn list(self: *ResponseEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *ResponseEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *ResponseEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *ResponseEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
