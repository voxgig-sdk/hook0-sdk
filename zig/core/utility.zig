...
pub fn make_error_util(ctx: *Context) E!Value {
    ...
    const the_err: *err.Hook0Error = in_err orelse (result.err orelse ctx.make_error("unknown", "unknown error"));

    const errmsg = the_err.msg;
    const msg0 = fmt("Hook0SDK: {s}: {s}", .{ opname, errmsg });
    ...
    const sdk_err = err.Hook0Error.make("", msg);
